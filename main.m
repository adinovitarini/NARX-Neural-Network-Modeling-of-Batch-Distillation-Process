clearvars -except data data_test
clc
clf
tic
U = data(:,1);
...
woh
% %Applied to training 
L_t = TimeDelayBlock(m_row,na,nb,U,Y);
L_t = L_t(1:m_row,:);
...
k_test = forward(m_row_test,wih,L_test);
a_test = forwardOut(m_row_test,woh,k_test);
...
subplot(212);
plot(t_t(1:m_row_test,1),a_test);
hold on
plot(t_t(1:m_row_test,1),Y_test,'r');
title('Testing set')
xlabel('time');
ylabel('normalize concentration')
legend('real data','estimated value')
